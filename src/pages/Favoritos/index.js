...
function Favoritos() {
  ...
  return (
    <div className="meus-filmes">
      ...
      <ul> {/* Lista de filmes */} 
        {filmes.map((item) => {  // Mapeando os filmes para exibir na tela
          return (
            <li key={item.id}>  {/* Key para identificar cada filme, */}
              <span>{item.title}</span> {/* Exibindo o título do filme */}

              <div>
              <Link to={`/filme/${item.id}`}>Ver detalhes</Link> {/* Link para acessar a página do filme */}
                <button>Excluir</button> {/* Botão para excluir o filme */}
              </div>
            </li> 
          );
        })}
